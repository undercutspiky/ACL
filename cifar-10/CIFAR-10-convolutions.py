    ...
    while i <= epochs:
        ...
        if cursor == 0:
            ...
            for iii in xrange(100):
                a = session.run([correct_], feed_dict={x: valid_x[iii*100:(iii+1)*100], y: valid_y[iii*100:(iii+1)*100]})
                cor_pred.append(a)
            print "Accuracy = "+str(np.mean(cor_pred))
            i += 1
    losses = np.array(losses); iterations = np.array(iterations);# activations = np.array(activations)
