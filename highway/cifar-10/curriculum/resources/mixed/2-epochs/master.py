...
def curriculum(difficult=False):
    t = [np.load("../../../../hrun-"+str(i+1)+"/transforms.npy") for i in xrange(3)]
    t.append(np.load("../../../../hrun-weird/transforms.npy"))
    t.append(np.load("../../../../hrun-2weird/transforms.npy"))
    t = [i.reshape(50000) for i in t]
...
with tf.Session(graph=graph) as session:
    ...
    sequence1 = np.random.choice(len(train_x), size=len(train_x), replace=False, p=curriculum(False))
    sequence2 = np.random.choice(len(train_x), size=len(train_x), replace=False, p=curriculum(True))
    seq = np.append(sequence1[:2500], sequence2[:2500])

    random_train_x = train_x[seq]
    random_train_y = train_y[seq]
    ...
    while i <= epochs:
        ...
        batch_ys = random_train_y[cursor: min((cursor + batch_size), len(random_train_x))]
        ...
        if cursor > len(random_train_x):
            ...
            test_acc.append(np.mean(cor_pred))
            tflearn.is_training(True, session=session)
            if i > 1:
                random_train_x = train_x[sequence2]
                random_train_y = train_y[sequence2]
            i += 1
    ...
    np.save('sequence1',sequence1)
    np.save('sequence2', sequence2)
