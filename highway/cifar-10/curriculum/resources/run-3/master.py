...
with tf.Session(graph=graph) as session:
    ...
    sequence = np.random.choice(len(train_x), size=len(train_x), replace=False, p=curriculum())

    random_train_x = train_x[sequence][:5000]
    random_train_y = train_y[sequence][:5000]
    ...
    while i <= epochs:
        ...
        if cursor > len(random_train_x):
            ...
            tflearn.is_training(True, session=session)
            random_train_x = train_x[sequence][:min(5000*(i+1), 50000)]
            random_train_y = train_y[sequence][:min(5000*(i+1), 50000)]
            i += 1
